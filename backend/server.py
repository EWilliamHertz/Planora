...
class NotificationCreate(BaseModel):
    type: str  # 'invitation_sent', 'event_reminder', 'task_assigned', etc.
    title: str
    message: str
...
async def create_notification(user_id: str, type: str, title: str, message: str, 
                            related_id: Optional[str] = None, data: Optional[Dict] = None):
    """Helper to create notifications when events happen"""
    notification_id = f"notif_{uuid.uuid4().hex[:12]}"
    now = datetime.now(timezone.utc).isoformat()
    async with db_pool.acquire() as conn:
        await conn.execute(
            "INSERT INTO notifications (notification_id, user_id, type, title, message, related_id, data, created_at) "
            "VALUES ($1, $2, $3, $4, $5, $6, $7, $8)",
            notification_id, user_id, type, title, message, related_id, data, now
        )
